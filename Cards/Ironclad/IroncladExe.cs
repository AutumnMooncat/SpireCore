...
internal sealed class IroncladExe : Card, IRCard
{
    public static string ID => nameof(IroncladExe);
    ...
    public override List<CardAction> GetActions(State s, Combat c)
    {
        return [
            ...
            {
                amount = GetChoices(),
                limitDeck = Characters.Ironclad.DeckEntry.Deck,
                makeAllCardsTemporary = true,
                overrideUpgradeChances = false,
                canSkip = false,
                inCombat = true,
                discount = -1,
                dialogueSelector = $".summon{Characters.Ironclad.DeckEntry.UniqueName}"
            }
        ];
    }
}
