...
public class DemonFormStatus : IRStatus, IKokoroApi.IV2.IStatusLogicApi.IHook, IKokoroApi.IV2.IStatusRenderingApi.IHook
{
    public static string ID => nameof(DemonFormStatus);
    ...
    public static void Register(IModHelper helper)
    {
        Entry = helper.Content.Statuses.RegisterStatus(ID, new()
        {
            Definition = new()
            {
                ...
                icon = CommonIcons.Find("demonForm"),
    ...
    public IReadOnlyList<Tooltip> OverrideStatusTooltips(IKokoroApi.IV2.IStatusRenderingApi.IHook.IOverrideStatusTooltipsArgs args)
    {
        ...
        ret.Add(new TTGlossary("status.shieldAlt"));
        ret.Add(new TTGlossary("status.heat", $"<c=boldPink>{args.Ship?.overheatDamage ?? 3}</c>"));
        return ret;
    }
