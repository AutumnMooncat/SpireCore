...
internal sealed class CrackedCore : Artifact, IRArtifact
{
    public static string ID => nameof(CrackedCore);
    ...
    {
        return [
            ITooltipHelper.MakeTooltip("stuff", LightningObject.ID, LightningObject.icon, Colors.drone, null, new {LightningObject.Damage}),
    ...
    public override void OnCombatStart(State state, Combat combat)
    {
        ...
        var list = source.Shuffle(state.rngActions).Take(1).ToList();
        foreach (int num in list)
        {
            var stuff = new LightningObject()
            {
                x = num,
                xLerped = num,
                targetPlayer = false
